...
public class TvRequestView extends VerticalLayout {
  ...
  public TvRequestView(TvRequestRepository tvRequestRepository, TvController tvController,
    ...
    Grid.Column<TvRequest> titleColumn = grid.addColumn(TvRequest::getTitle).setHeader("Title")
        .setAutoWidth(true).setSortable(true).setComparator(Comparator.comparing(TvRequest::getTitle,
    ...
    grid.setItemDetailsRenderer(new ComponentRenderer<>(TvRequestView::createDetails));
    ...
    GridMenuItem<TvRequest> markAvailableItem = contextMenu.addItem("Mark Available",
  ...
  private void refreshGrid() {
    ...
    noteRepository.findAll().forEach(n -> tvRequestsWithNotes.add(n.getRequest().getId()));
    ...
    long withNotesCount = all.stream().filter(mr -> tvRequestsWithNotes.contains(mr.getId())).count();
  ...
  private com.vaadin.flow.component.Component ombiLink(TvRequest mr) {
    ...
    return externalLink(ombiUrl + "/details/tv/" + tvdbId);
  }
  ...
  private static FormLayout createDetails(TvRequest mr) {
